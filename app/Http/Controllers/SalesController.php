...
public function verifySalePayment(Request $request)
{
    ...
    try {
        ...
        $bakongData = $verify['data'] ?? null;

        // Payment not found or unsuccessful
        if (($verify['responseCode'] ?? 1) !== 0 || empty($bakongData)) {
            return response()->json([
                'status'  => false,
                'message' => 'Payment not found or unsuccessful',
                'bakong'  => $verify,
            ], 200); // ✅ Use 200 to avoid Axios error
        }
        ...
        if ($sale->payment_status === 'paid') {
            return response()->json([
                ...
                'sale'    => $sale,
                'bakong'  => $bakongData, // full Bakong info
            ], 200);
        }

        // Check if payment acknowledged
        if (!empty($bakongData['acknowledgedDateMs'])) {
            // Mark sale as paid
            $sale->update([
                'payment_status' => 'paid',
                'status'         => 'paid',
        ...
        // Payment not yet scanned
        return response()->json([
            'status'  => false,
            'state'   => 'pending',
            'message' => 'Payment pending. User has not scanned yet.',
            'sale'    => $sale,
            'bakong'  => $bakongData,
        ], 200);
